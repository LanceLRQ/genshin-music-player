...
`timing` 已按游戏内实测（60 帧上限）校准回写：`holdMs` 统一为 30；`minRepeatGapMs` 诗琴类（风物 / 镜花 / 老旧 / 谐律 / 跃律）为 75，鼓与和弦吉他（悠可 / 余音 / 三件鼓）为 96，晚风圆号为 120，沃雅妮莎暂为初始值 40。换设备或改帧率后如果出现漏音，按[开发文档](../development.md)的校准方法重新测量。晚风圆号（圆号）与沃雅妮莎（人声）在游戏里按住会持续发声，`sustain` 均为 `true`（沃雅妮莎 2026-09-23 实测按音长演奏效果最好）；其余内置乐器按住无效，`sustain` 为 `false`。
