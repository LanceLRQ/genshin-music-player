...
| `evening-horn` | 晚风圆号 | 音高类 | 待实测 | 两行 14 键：高音谱号行 `Q W E R T Y U` = 60–71；低音谱号行 `A S D F G H J` = 48–59 |
| `yuco-lyre` | 悠可琴 | 音高类 | 待实测 | 和弦吉他：第一排 `Q W E R T Y U` 为和弦键 C、Dm、Em、F、G、Am、Bdim；中音 / 低音两排单音同诗琴布局 |
| `lingering-echo` | 余音 | 音高类 | 待实测 | 和弦吉他，布局同悠可琴 |
| `harmony-clavier` | 谐律键琴 | 音高类 | 待实测 | 三行 21 键，布局同风物之诗琴 |
| `sprightly-lyre` | 跃律琴 | 音高类 | 待实测 | 三行 21 键，布局同风物之诗琴 |
| `festive-drum` | 荒泷·盛世豪鼓 | 敲击类 | 已验证 | 2 键：咚 = `S`，咔 = `A` |
| `banquet-drum` | 绮筵之鼓 | 敲击类 | 已验证 | 4 键 `A S K L` = 咔、咚、咚-2、咔-2 |
| `juju-drum` | 聚聚鼓 | 敲击类 | 待实测 | 8 键四组音色：上排 `Q W I O`、下排 `A S K L` 分别为 bass、tom、snare、ride 及其 `-2` 变体 |

标记"待实测"的乐器可以正常使用，只是键位或音高可能与游戏实际不符；用户可以自己编辑校准，不需要等待软件更新。
