...
| `two-row-prototype` | 沃雅妮莎 | 音高类 | 人声 | 已验证 | 两行 14 键：高音谱号行 `Q W E R T Y U` = 60 62 64 65 67 69 71；低音谱号行 `A S D F G H J`（la 在 H）= 48 50 52 53 55 57 59（2026-09-23 用户实测修正，原截图误读为 N） |
...
`timing` 已按游戏内实测（60 帧上限）校准回写：`holdMs` 统一为 30；`minRepeatGapMs` 诗琴类（风物 / 镜花 / 老旧 / 谐律 / 跃律）为 75，鼓与和弦吉他（悠可 / 余音 / 三件鼓）为 96，晚风圆号为 120，沃雅妮莎暂为初始值 40。换设备或改帧率后如果出现漏音，按[开发文档](../development.md)的校准方法重新测量。晚风圆号、沃雅妮莎在游戏里按住都会持续发声：晚风圆号 `sustain` 为 `true`；沃雅妮莎尾音本来就长，`sustain` 为 `false`（默认按固定按住时长），需要按音长演奏可以在演奏页手动打开「音长按键」。其余内置乐器按住无效，`sustain` 为 `false`。
