...
- **多轨**：可以选择多条音轨合并演奏，也可以单独试听或演奏某一轨；合并时同一个键上的同音只按一次。
...
- [x] Tauri 应用与界面（2026-09-18 完成：应用壳、界面框架、演奏页、乐器页与设置页；含 macOS CGEvent 演奏后端与「模拟发声」模式；2026-09-19 虚拟琴键卡支持键帽 / 落音双视图切换）
- [x] 使用文档与测试素材（2026-09-18 完成：格式文档、音频转 MIDI 指南、测试 MIDI 生成脚本；2026-09-19 补完整用户手册 `docs/guides/user-guide.md`，instrument-profile 内置乐器表同步 12 件现状）
- [ ] Windows 真机验证：无游戏链路层完成（2026-09-18），游戏内 12 件乐器 11 件完成（2026-09-19：计时/MIDI 实奏/逐乐器校准与写回——诗琴类 75ms、鼓与和弦吉他 96ms、晚风圆号 120ms；荒泷鼓/晚风圆号/聚聚鼓键位按玩家现场报告修正；发现普通权限注入被内核过滤、须管理员运行——README 已说明）；UAC 1.2–1.3 与 7.4 热键、§7.2 暂停恢复闭环（2026-09-19 晚）；沃雅妮莎未上线跳过；音高录音定标、§7.7 循环听感、7.5 游戏提权组合待续
...
docs/                     使用者文档（键盘谱、简谱、乐器配置格式，音频转 MIDI 指南）
